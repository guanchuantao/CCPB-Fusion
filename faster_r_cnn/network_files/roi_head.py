...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    """
    计算Faster R-CNN的头部损失函数
    :param class_logits: 预测的类别概率信息，shape=[num_anchors, num_classes]
    :param box_regression: 预测的边界框回归信息
    :param labels: 真实的类别标签
    :param regression_targets: 真实的边界框回归目标
    :return:
    """
    # 将批次中所有图像的标签和回归目标拼接成一个张量
    ...
    # 重塑边界框回归结果的形状 [N, num_classes*4] -> [N, num_classes, 4]
    box_regression = box_regression.reshape(N, -1, 4)
    # 计算边界框回归损失（平滑L1损失）
    box_loss = det_utils.smooth_l1_loss(
        # 获取指定索引proposal的指定类别box信息，只计算正样本的边界框回归损失
        box_regression[sampled_pos_inds_subset, labels_pos],
        regression_targets[sampled_pos_inds_subset],
        beta=1 / 9,     # 平滑L1的beta参数
        size_average=False,  # 不自动求平均
    ) / labels.numel()      # 除以总样本数进行归一化
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        """
        为每个proposal匹配对应的gt_box，并划分为正/负样本
        :param proposals:
        :param gt_boxes:
        :param gt_labels:
        :return:
        ...
        # 存储分配的标签
        labels = []
        # 遍历每张图像里面的proposals, gt_boxes, gt_labels信息
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                # 计算proposal与每个gt_box匹配的iou最大值，并记录索引，IOU小于低阈值为-1，IoU在两个阈值之间为-2，IOU大于低阈值为0
                ...
                # 标记忽略样本 (低阈值 <= IOU < 高阈值)，将gt索引为-2的类别设置为-1, 即废弃样本
    ...
    def postprocess_detections(self,
        ...
        :param box_regression: 网络预测的边界框回归参数
        :param proposals: rpn输出的proposal
        :param image_shapes: 打包成batch前每张图像的宽高
        Returns:
        ...
        all_boxes = []      # 存储每张图像的边界框
        all_scores = []     # 存储每张图像的置信度
        all_labels = []     # 存储每张图像的标签
        # 遍历每张图像预测信息
    ...
    def forward(self,
        ...
        """
        头部预测
        :param features: FPN之后的特征
        :param proposals: 经历过RPN阶段之后选择的anchors，叫做proposals
        :param image_shapes: 每张图像的宽高
        :param targets: 图像对应的目标
        :return:
        """
...
# 2、将采集样本通过Multi-scale RoIAlign pooling层
        # 输入:
        #   features: 特征金字塔字典 {level: feature_map}
        #   proposals: 候选框列表 (每张图像[N, 4])
        #   image_shapes: 图像原始尺寸列表
        # 输出: [num_proposals, channels, 7, 7]
        box_features = self.box_roi_pool(features, proposals, image_shapes)

        # 3、通过roi_pooling后的两层全连接层
        # 输入: [num_proposals, channels, 7, 7]
        # 输出: [num_proposals, representation_size] (representation_size通常为1024)
        box_features = self.box_head(box_features)

        # 4、接着分别预测目标类别和边界框回归参数
        # class_logits: [num_proposals, num_classes]
        # box_regression: [num_proposals, num_classes * 4]
        ...
        if self.training:
            ...
            # 5. 计算损失（边界框回归损失只计算正样本的损失，并且总损失除以样本，为平均损失；分类损失计算所有）
