...
def resnet101_fpn_backbone(pretrain_path="",
    ...
    resnet101_backbone = ResNet(Bottleneck,  # 使用Bottleneck块
                                [3, 4, 23, 3],  # 各stage的block数量 [3,4,6,3]对应ResNet50
    ...
    if isinstance(norm_layer, FrozenBatchNorm2d):
        overwrite_eps(resnet101_backbone, 0.0)
    # 加载预训练权重
    if pretrain_path != "":
        ...
        print(resnet101_backbone.load_state_dict(  # 将加载的权重字典应用到模型中
    ...
    for name, parameter in resnet101_backbone.named_parameters():
    ...
    in_channels_stage2 = resnet101_backbone.in_channel // 8  # 256
    # 各层输入通道数列表 [256, 512, 1024, 2048]对应layer1-4
    in_channels_list = [in_channels_stage2 * 2 ** (i - 1) for i in returned_layers]
    # FPN输出通道数(统一为256)
    out_channels = 256
    return BackboneWithFPN(resnet101_backbone, return_layers, in_channels_list, out_channels, extra_blocks=extra_blocks)
